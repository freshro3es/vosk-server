...
def handle_listen_task(data):
    ...
    logging.info(
        f"listen task request: task id is {task_id} and client sid is {request.sid}"
    )
    ...
    if task_id:
        ...
        if task:
            ...
            logging.info(
                f"listen_task event: started work on WAV file {task.filename} with id {task_id}"
            )
            socketio.start_background_task(target=task.transcribe_file)
            send_message(
                request.sid, "listening", {"message": f"Listening to task {task_id}"}
            )
        else:
...
def handle_start_recording(data):
    ...
    logging.info(
        f"Recording started, num channels is {channel_count} and sample rate is {sample_rate}"
    )
    task_manager = current_app.config["TASK_MANAGER"]
    task = task_manager.add_voice_task(request.sid, sample_rate, channel_count, 2)
    logging.info(
        f"Client {task.client_sid} created a file. Audio file path: {task.audio_file_path}"
    )
